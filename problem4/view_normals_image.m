function view_normals_image(image)
    ...
    change_callback_function = {@image_cick, imageHandle, hSlider, image, image_gray, tot_row, tot_col};
...
function image_cick(~, ~, imageHandle, hSlider, image, image_gray, tot_row, tot_col)
    c_row = tot_row / 2;
    c_col = tot_col / 2;
    ...
    for i=1:1:tot_row
        for j=1:1:tot_col
			...
			image_gray(i, j) = 255 * (light(1)*x + light(2)*y + light(3)*z);
		end
    end
